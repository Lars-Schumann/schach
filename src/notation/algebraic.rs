...
impl Move {
    ...
    pub fn san(self) -> Vec<AsciiChar> {
        ...
        let interfering_moves = legal_moves
            .iter()
            .filter(|legal| legal.kind.piece_kind() == self.kind().piece_kind())
            .filter(|legal| legal.destination == self.destination())
            .filter(|legal| {
                // for the Promotion case, remove Duplicate Promotions to just different pieces.
                if self.kind().is_promotion()
                    && legal.origin == self.origin()
                    && legal.destination == self.destination()
                {
                    return false;
                }
                true
        ...
        if interfering_moves.is_empty() {
            if self.kind().piece_kind() == PieceKind::Pawn && self.inner().is_capture() {
        ...
        if not(interfering_moves
            .iter()
            .any(|inter| inter.origin.row == self.origin().row))
        {
        ...
        if not(interfering_moves
            .iter()
            .any(|inter| inter.origin.col == self.origin().col))
        {
            return notation_creator(self, OriginAmbiguationLevel::FileOnly, capture_repr);
        }
